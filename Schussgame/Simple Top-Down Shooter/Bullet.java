...
public class Bullet extends NPC //Shoot from the Player to kill enemys.
{
    ...
    public void Kill_Slime() //Kill's Slimes it touches
    {
        if(this.isTouching(Slime.class)) // Makes Shure it touches a Slime
        {
            ...
            else if(Greenfoot.getRandomNumber(101)>80&&MyWorld.forcefield_active/MyWorld.game_speed<180) //Spawns a Forcefield if no upgrade was spawned, based on lucḱ and you have less then 180 seconds of Forcefield time left.
            {
            ...
            if(Greenfoot.getRandomNumber(101)>70&&MyWorld.Hearts<5) //If you have less then 5 hearts it can randomly spawn a Medikit.
            {
                getWorld().addObject(new MediKit(), getOneIntersectingObject(Slime.class).getX(), getOneIntersectingObject(Slime.class).getY());
            }
            else if(Greenfoot.getRandomNumber(101)>95&&MyWorld.Hearts<10) //If you have less than 10 Hearts it is less likely to spawn a Medikit.
            {
                getWorld().addObject(new MediKit(), getOneIntersectingObject(Slime.class).getX(), getOneIntersectingObject(Slime.class).getY());
            }
